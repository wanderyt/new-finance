...
### Added
- **Searchable Brand Dropdown**: The fin item panel brand field now uses the shared searchable dropdown pattern, with suggestions populated from the user's previously saved item brand names

### Changed
- **Autocomplete Data**: `/api/fin/autocomplete` now returns distinct item brands alongside merchant, place, and city suggestions for reuse in line item dialogs
...
### Added
- **Expense Transaction Type for Pocket Money**: New '花销' (expense) type to track daily spending
  - Differentiates regular expenses from punishments for clearer allowance tracking
  - Orange color scheme (💸) with distinct visual styling
  - Can be created, edited, and deleted via UI
  - Negative amount like deductions, but semantically different (spending vs punishment)
  - Database migration to update CHECK constraint
  - Updated TypeScript types across all layers
  - Full API validation support
